...
const CreateCapsule = () => {
  ...
  const handleSubmitCapsule = async () => {
    ...
    const filesToFinalize = uploadedFiles.filter(f => f.cloudinaryUrl && f.uploaded);
    ...
    const mediaUrlsForApi = filesToFinalize.map(file => ({
      url: file.cloudinaryUrl,
      publicId: file.publicId, // Crucial: Make sure MediaUploader provides this
      name: file.name,
    ...
    const capsulePayload = {
      ...
      unlockDate: capsuleData.unlockDate.toISOString(), // Ensure ISO string format
      isPublic: capsuleData.isPublic,
      tags: capsuleData.tags.filter(tag => tag.trim() !== ''), // Ensure no empty tags
    ...
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.msg || 'Failed to seal the capsule');
      }
    } catch (error) {
